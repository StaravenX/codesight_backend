 ...
 * 基于 Redis 的刷新令牌白名单存储。
 * <p>
 * 键空间：`auth:refresh_token:{userId}:{tokenId}`，值固定为 "1"，设置 TTL 控制过期。
...
public class RedisRefreshTokenStore implements RefreshTokenStore {
    ...
    public void revokeAll(long userId) {
        String pattern = "auth:refresh_token:%d:*".formatted(userId);
    ...
    private static String key(long userId, String tokenId) {
        return "auth:refresh_token:%d:%s".formatted(userId, tokenId);
    }
}
